...
function init() {
  ...
  // Громкое предупреждение: в проде DEV-режим означает, что коды подтверждения
  // уходят в лог сервера, а не в почту. Регистрация при этом «проходит», человек
  // видит экран ввода кода — и код не приходит. Ошибка молчаливая и очень дорогая
  // по времени поиска, поэтому кричим в лог при каждом старте.
  if (mode === 'dev' && process.env.NODE_ENV === 'production') {
    console.warn(
      [
        '',
        '!'.repeat(60),
        '  ВНИМАНИЕ: продакшн без почты — коды подтверждения НЕ отправляются.',
        '  Письма не уходят, код печатается только в логе сервера.',
        '  Люди регистрируются, но подтвердить почту не могут.',
        '',
        '  Задай в панели хостинга (Render → Environment) одну из пар:',
        '    SMTP_HOST=smtp.gmail.com  SMTP_PORT=465  SMTP_SECURE=true',
        '    SMTP_USER=<адрес>         SMTP_PASS=<пароль приложения, 16 символов>',
        '  либо RESEND_API_KEY=<ключ>.',
        '  После сохранения перезапусти сервис и проверь /healthz — там должно быть',
        '  "mail":"smtp" вместо "mail":"dev".',
        '!'.repeat(60),
        ''
      ].join('\n')
    );
  ...
  // Хост задан, а логина нет — SMTP-сервер (в частности Gmail) такой вход отвергнет.
  ...
  if (mode === 'smtp' && !SMTP_PASS) {
    console.warn('[mail] SMTP_USER задан, но SMTP_PASS пуст — нужен пароль приложения, письма не уйдут.');
  }
